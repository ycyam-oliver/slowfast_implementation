...
from slowfast.visualization.video_visualizer import get_class_names # Oliver
...
class Predictor:
    ...
    def __init__(self, cfg, gpu_id=None):
        ...
        if cfg.DETECTION.ENABLE:
            # self.object_detector = Detectron2Predictor(cfg, gpu_id=self.gpu_id) # Oli
            self.object_detector = YOLOPredictor(cfg, gpu_id=self.gpu_id) # Oli
...
class YOLOPredictor:
    ...
    def __init__(self, cfg, gpu_id=None):
        # 加载预训练的 YOLOv8n 模型
        self.model = YOLO("/home/pigs/YOLO_models/runs/detect/pigs-behaviours2/weights/best.pt")#cfg.DEMO.YOLO_MODEL) # Oli # to be specified in the yaml file, e.g./home/oliver/computer_vision/classification/SlowFast/demo/AVA/SLOWFAST_32x2_R101_50_50_YOLO.yaml
        self.detect_names, _, _ = get_class_names(cfg.DEMO.LABEL_FILE_PATH, None, None)#cfg.DEMO.Detect_File_Path, None, None) # Oli
    ...
    def __call__(self, task):
        ...
        """加入预测标签"""
        task.add_action_preds(preds)#add_detect_preds(preds)
        task.add_bboxes(pred_boxes)
